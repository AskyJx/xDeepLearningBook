...
class DemoData(object):
    ...
    # 返回nx1 随机整数向量
    def getVecInt(self,n):
        return np.random.randint(-10,10,(n,1))

    # 返回nxm的随机整数矩阵
    def getMatrixInt(self,n,m):
        return np.random.randint(-10,10,(n,m))

    def getMatrixf(self,n,m,dataType):
...
def main():
    ...
    # 数据对象初始化
    # mnist = MnistData(path_minst_unpack, is4Cnn=True, Params.DTYPE_DEFAULT)
    ...
    fc1 = FCLayer(Params.MINI_BATCH_SIZE, Params.IMAGE_SIZE_L, Params.TYPE_K, NoAct,
    ...
    for epoch in range(Params.EPOCH_NUM):
        ...
        for batch in range(len(dataRngs)):
            ...
            if iter < 9:
                w.append(fc1.w.T.copy())  # 添加前9次
            for iteration in range(Params.ITERATION):
                acc_t, loss_t = sess.train_steps(x, y_, lrt)
                iter += 1
            if (batch % Params.LOG_FREQ == 0 or iter<9):  # FREQ个batch show一次日志
    ...
    view.show()
    w.append(fc1.w.T.copy()) # 和最后一次训练的结果
    view.v2img(np.array(w))
